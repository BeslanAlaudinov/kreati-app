...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updatePlacement = (forceHeight) => {
        if (!containerRef.current || !renderer) return;

        const newWidth = containerRef.current.clientWidth;

        // На мобилке игнорируем resize если ширина не изменилась
        // (это значит что просто адресная строка спряталась/показалась)
        if (!forceHeight && newWidth === lastWidthRef.current) return;
      ...
      updatePlacement(true); // первый раз — форсируем
      animationIdRef.current = requestAnimationFrame(loop);
